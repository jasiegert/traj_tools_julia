...
function rdf(coord, atom, atomtype1, atomtype2, pbc, d_min, d_max, bins)
    ...
    create_dist_histogram!(dist_histogram, coord, atom, atomtype1, atomtype2, pbc, distance_limits)
    return distance_mean, dist_histogram ./ (shell_volumes .* ideal_density .* size(coord)[3])
end

function create_dist_histogram!(dist_histogram, coord, atom, atomtype1, atomtype2, pbc, distance_limits)
    ...
    for i in 1:size(coord)[3]
        for x in eachcol(coord1[:, :, i])
            for y in eachcol(coord2[:, :, i])
            distance = pbc_dist_inv!(dist_tmp, x, y, pbc, inv_pbc)
...
function create_dist_histogram_multi(coord, atom, atomtype1, atomtype2, pbc, distance_limits)
    ...
    Threads.@threads for i_thread in 1:Threads.nthreads()
        ...
        @views for i in i_limits
            ...
            for x in eachcol(coord1[:, :, i])
                for y in eachcol(coord2[:, :, i])
                    distance = pbc_dist_inv_matmul!(dist_tmp, matmul_tmp, x, y, pbc, inv_pbc)
                    bin_multi!(dist_histogram, distance, d_min, d_max, d_step)
                end
            end
        end
...
function bin_multi!(dist_histogram, distance, d_min, d_max, d_step)
